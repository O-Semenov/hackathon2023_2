...
class Forklift:
    ...
    def get_task(self):
        ...
        if self.task:
            ...
            response = requests.post('http://127.0.0.1:5000/script', json=json.dumps(data))
            ...
            data = {
                ...
                "warehouse_id": self.warehouse_id,
                "task_id": self.task.id,
                "point_id": self.current_point['check_point_name'],
                "target_id":"",
                "time": datetime.now().isoformat(),
            }
            response = requests.post('http://127.0.0.1:5000/script', json=json.dumps(data))
        ...
        if self.status == 'working':
            ...
            if self.next_point_time:
                ...
                if datetime.now() > self.next_point_time:
                    ...
                    if self.next_point:
                        print(
                            f"ХЙForklift #{self.id} of warehouse #{self.warehouse_id} "
                        ...
                        data = {
                            ...
                            "warehouse_id": self.warehouse_id,
                            "task_id": self.task.id,
                            "point_id": self.next_point['check_point_name'],
                            "target_id": "",
                            "time": datetime.now().isoformat(),
                        }
                        response = requests.post('http://127.0.0.1:5000/script', json=json.dumps(data))
                        ...
                        try:
                            ...
                            if self.path_direction == "forward":
                                print(
                                    f"НЕ ХЙForklift #{self.id} of warehouse #{self.warehouse_id} "
                                ...
                                data = {
                                    ...
                                    "task_id": self.task.id,
                                    "point_id": "",
                                    "target_id": self.current_path.target_rack_id,
                                    "time": datetime.now().isoformat(),
                                }
                                response = requests.post('http://127.0.0.1:5000/script', json=json.dumps(data))
                    ...
                    # if we reach this else condition then forklift finish the task
                        ...
                        print(
                            f"вс ХЙForklift #{self.id} of warehouse #{self.warehouse_id} "
